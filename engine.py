...
def ark_engine(
    ...
    num_classes_list = [
        len(datasets_config[dataset]["diseases"]) for dataset in dataset_list
    ]
    ...
    if args.ema_mode == "epoch":
        momentum_schedule = cosine_scheduler(
            args.momentum_teacher, 1, args.pretrain_epochs, len(dataset_list)
        )
        coef_schedule = cosine_scheduler(
            0, 0.5, args.pretrain_epochs, len(dataset_list)
        )
    ...
    if args.resume:
        ...
        if os.path.isfile(resume):
            ...
            print(
                "=> loaded checkpoint '{}' (epoch={:04d}, val_loss={})".format(
                    resume, start_epoch, init_loss
                )
            )
            start_epoch += 1
        else:
    ...
    for epoch in range(start_epoch, args.pretrain_epochs):
        ...
        print(
            "Epoch {:04d}: avg_val_loss {:.5f}, saving model to {}".format(
                epoch, avg_val_loss, save_model_path
            )
        )
        save_checkpoint(
            {
            ...
            with open(output_file, "a") as writer:
                ...
                for i, dataset in enumerate(dataset_list):
                    ...
                    y_test, p_test = test_classification(
                        model, i, data_loader_list_test[i], device, multiclass
                    )
                    y_test_teacher, p_test_teacher = test_classification(
                        teacher, i, data_loader_list_test[i], device, multiclass
                    )
                    if multiclass:
                        acc = accuracy_score(
                            np.argmax(y_test.cpu().numpy(), axis=1),
                            np.argmax(p_test.cpu().numpy(), axis=1),
                        )
                        acc_teacher = accuracy_score(
                            np.argmax(y_test_teacher.cpu().numpy(), axis=1),
                            np.argmax(p_test_teacher.cpu().numpy(), axis=1),
                        )
                        print(
                            ">>{}:Student ACCURACY = {}, \nTeacher ACCURACY = {}\n".format(
                                dataset, acc, acc_teacher
                            )
                        )
                        writer.write(
                            "\n{}: Student ACCURACY = {}, \nTeacher ACCURACY = {}\n".format(
                                dataset,
                                np.array2string(
                                    np.array(acc), precision=4, separator="\t"
                                ),
                                np.array2string(
                                    np.array(acc_teacher), precision=4, separator="\t"
                                ),
                            )
                        )
                        t_res.append(acc)
                    ...
                    if dataset == "CheXpert":
                        test_diseases_name = datasets_config["CheXpert"][
                            "test_diseases_name"
                        ]
                        ...
                        individual_results_teacher = metric_AUROC(
                            y_test_teacher, p_test_teacher, len(test_diseases)
                        )
                    else:
                        individual_results = metric_AUROC(y_test, p_test, len(diseases))
                        individual_results_teacher = metric_AUROC(
                            y_test_teacher, p_test_teacher, len(diseases)
                        )
                    print(
                        ">>{}:Student AUC = {}, \nTeacher AUC = {}\n".format(
                            dataset,
                            np.array2string(
                                np.array(individual_results),
                                precision=4,
                                separator="\t",
                            ),
                            np.array2string(
                                np.array(individual_results_teacher),
                                precision=4,
                                separator="\t",
                            ),
                        )
                    ...
                    mean_over_all_classes_teacher = np.array(
                    ...
                    writer.write(
                        "{}: Student mAUC = {:.4f}, Teacher mAUC = {:.4f}\n".format(
                            dataset,
                            mean_over_all_classes,
                            mean_over_all_classes_teacher,
                        )
    ...
    with open(output_file, "a") as writer:
        writer.write(
            "Omni-pretraining stage: \nStudent meanAUC = \n{} \nTeacher meanAUC = \n{}\n".format(
                np.array2string(np.array(test_results), precision=4, separator="\t"),
                np.array2string(
                    np.array(test_results_teacher), precision=4, separator="\t"
                ),
            )
        )
    writer.close()
