...
class ArkSwinTransformer(swin.SwinTransformer):
    def __init__(
        self, num_classes, projector_features=None, use_mlp=False, *args, **kwargs
    ):
...
def build_omni_models_from_checkpoint(args, num_classes_list, key):
    models = []
    if args.model_name == "swin_base":  # swin_base_patch4_window7_224
        for num_classes in range(len(num_classes_list) + 1):
            models.append(
                ArkSwinTransformer(
                    num_classes,
    ...
    if args.pretrained_weights is not None:
        ...
        for model in models:
            ...
            print("Loaded with model {} msg: {}".format(i, msg))
            i += 1
